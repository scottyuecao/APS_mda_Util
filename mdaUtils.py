#==============================================================================
# Data processing for time-resolved experiments at SACLA
# 2020-02-28
# General codes for the sync mode. One h5 per combined motor position
# 
# Ver. 1
# Yue Cao (ycao@colorado.edu)
#
# After Pviewer - A Packaged Array Data Viewer Written in Python Language
# by Ben-chin K Cha & Timothy Mooney
# Maintained by Ben-chin Cha (cha@aps.anl.gov) 
# Ref: https://epics.anl.gov/bcda/dataVis/pviewer.html#HDR1_1
# 
# Rewritten from readMDA.py, version 4 of Tim Mooney 8/22/02
# Main updates:
# 1) Updated to py3 from py2
# 2) Updated names of certain objects and variables to avoid conflict with
#    the names of data types
# 3) Removed interactive GUI and main program
# 4) Renamd readScan to _readScan as an internal method
# 4) Renamd readMDA to _readMDA as an internal method
#
...
def _readScan(file, v, new=0):
    '''
    _readScan(file, v, new=0) - internal scan read routine, it unpack a subset of scan data from
    ...
    if v:
        print("scan.rank = {}".format(scan.rank))
    scan.npts = u.unpack_int()
    if v:
        print("scan.npts = {}".format(scan.npts))
    scan.curr_pt = u.unpack_int()
    if v:
        print("scan.curr_pt = {}".format(scan.curr_pt))
    if (scan.rank > 1):
        ...
        if v:
            print("scan.plower_scans = {}".format(scan.plower_scans))
    namelength = u.unpack_int()
    scan.name = u.unpack_string()
    if v:
        print("scan.name = {}".format(scan.name))
    timelength = u.unpack_int()
    scan.time = u.unpack_string()
    if v:
        print("scan.time = {}".format(scan.time))
    scan.np = u.unpack_int()
    if v:
        print("scan.np = {}".format(scan.np))
    scan.nd = u.unpack_int()
    if v:
        print("scan.nd = {}".format(scan.nd))
    scan.nt = u.unpack_int()
    if v:
        print("scan.nt = {}".format(scan.nt))
...
def _readMDA(fname, maxdim=2, verbose=1, help=0, new=0):
    '''
    _readMDA(fname, maxdim=2, verbose=1, help=0) - This fuction read an MDA file and 
    ...
    from mdaUtils import _readMDA
    d = _readMDA('/home/beams/CHA/data/xxx/cha_0001.mda')
    ...
    s0 = _readScan(file, max(0,verbose-1),new=new)
    dim[0].npts = s0.npts
    ...
    if ((rank > 1) and (maxdim > 1)):
        ...
        for i in range(s0.curr_pt):
            ...
            s = _readScan(file, max(0,verbose-1),new=new)
            if i == 0:
                dim[1].npts = s.npts
    ...
    if ((rank > 2) and (maxdim > 2)):
        ...
        for i in range(s0.curr_pt):
            file.seek(s0.plower_scans[i])
            s1 = _readScan(file, max(0,verbose-1),new=new)
    ...
    dictOut = {}
    dictOut['sampleEntry'] = ("description", "unit string", "value")
    dictOut['filename'] = fname
    dictOut['rank'] = rank
    dictOut['dimensions'] = dimensions
    if pExtra:
        file.seek(pExtra)
            ...
            if datatype == 0: # DBR_STRING
                ...
                for i in range(len(v)):
                    ...
                    if v[i] == 0:
                        break
                    value = value + chr(v[i])
            elif datatype == 29: # DBR_CTRL_SHORT
                value = u.unpack_farray(count, u.unpack_int)
            elif datatype == 33: # DBR_CTRL_LONG
                value = u.unpack_farray(count, u.unpack_int)
            elif datatype == 30: # DBR_CTRL_FLOAT
                value = u.unpack_farray(count, u.unpack_float)
            elif datatype == 34: # DBR_CTRL_DOUBLE
                value = u.unpack_farray(count, u.unpack_double)

            dictOut[name] = (desc, unit, value)
...
def readMDA(fname, verbose=0):
    '''
    Extracting information from a MDA
    '''
    mdata = _readMDA(fname, verbose=0)
    ...
    outPV = {}
    for i in range(numPositioner):
        key = 'P{}: {}'.format(i+1, mdata[-1].p[i].name.decode("utf-8"))
        outPV[key] = np.array(mdata[-1].p[i].data)

    for i in range(numDet):
        key = 'D{}: {}'.format(i+1, mdata[-1].d[i].name.decode("utf-8"))
        outPV[key] = np.array(mdata[-1].d[i].data)

    if verbose:
        for key in outPV.keys():
            print(key)
